...
import type { ExecutionPattern, AgentId } from './types.js';
...
function assertLessThan(actual: number, expected: number, testName: string): void {
  assert(actual < expected, `${testName} (${actual} < ${expected})`);
}
...
const pattern1: ExecutionPattern = {
  id: 'test_pattern_1',
...
const pattern2: ExecutionPattern = {
  id: 'test_pattern_2',
  objective: 'Run build',
...
const pattern2Failure = failures.find((f: any) => f.pattern_id === 'test_pattern_2');
...
const testAgents: AgentId[] = ['hollowed_eyes' as AgentId, 'the_curator' as AgentId];
...
await conflictDetector.learnConflict('hollowed_eyes' as AgentId, 'the_curator' as AgentId, 'tool_overlap', false);
...
const similarPattern1: ExecutionPattern = {
  id: 'kdtree_test_1',
...
const similarPattern2: ExecutionPattern = {
  id: 'kdtree_test_2',
...
const differentPattern: ExecutionPattern = {
  id: 'kdtree_test_3',
...
const allRecent = windowPatterns.every((p: any) => p.timestamp >= sevenDaysAgo);
