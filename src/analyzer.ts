import type { ProjectContext, AnalysisResult, Issue, Recommendation, AgentId } from './types.js';
 ...
 */
export function analyzeProject(context: ProjectContext): AnalysisResult {
  const critical_issues: Issue[] = [];
 ...
 */
function analyzeTests(testResults: any): { issues: Issue[], score_penalty: number } {
  const issues: Issue[] = [];
  let penalty = 0;
 ...
 */
function analyzeBuild(buildStatus: string): { issues: Issue[], score_penalty: number } {
  const issues: Issue[] = [];
  let penalty = 0;
 ...
 */
function analyzeLinearIssues(linearIssues: any[]): { issues: Issue[], score_penalty: number } {
  const issues: Issue[] = [];
  let penalty = 0;
 ...
 * Analyzes git status
 */
function analyzeGitStatus(gitStatus: string): { issues: Issue[], score_penalty: number } {
  const issues: Issue[] = [];
  let penalty = 0;
 ...
 */
function analyzeErrors(errors: any[]): { issues: Issue[], score_penalty: number } {
  const issues: Issue[] = [];
  let penalty = 0;
  ...
  const securityErrors = errorDetails.filter(e => 
    e.type?.toLowerCase().includes('security') || 
 ...
 */
function classifyErrorSeverity(error: any): 'critical' | 'high' | 'medium' | 'low' {
 ...
 */
function generateRecommendation(issue: Issue): Recommendation {
 ...
 */
function determineSuggestedAgents(issues: Issue[]): AgentId[] {
