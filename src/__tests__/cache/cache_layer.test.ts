...
import { CacheLayer, type CachedEntry, type CacheStats } from '../../knowledge/cache/cache_layer.js';
import { promises as fs } from 'fs';
import * as path from 'path';
import { CACHE_CONFIG } from '../../knowledge/cache/cache_config.js';

describe('CacheLayer', () => {
  let cacheLayer: CacheLayer<string>;
  ...
  beforeEach(async () => {
    cacheLayer = new CacheLayer<string>(testNamespace);
  ...
  describe('L2 Disk Cache', () => {
    test('should persist to disk and reload', async () => {
      ...
      const newCache = new CacheLayer<string>(testNamespace);
    ...
    test('should handle disk cache corruption gracefully', async () => {
      ...
      const corruptedCache = new CacheLayer<string>(testNamespace);
  ...
  describe('Complex Data Types', () => {
    test('should cache objects', async () => {
      ...
      const objectCache = new CacheLayer<TestObject>('test_objects');
    ...
    test('should cache arrays', async () => {
      const arrayCache = new CacheLayer<string[]>('test_arrays');
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle concurrent operations', async () => {
      ...
      const results = await Promise.all(
        Array.from({ length: 10 }, (_, i) => cacheLayer.get(`key${i}`))
      );
