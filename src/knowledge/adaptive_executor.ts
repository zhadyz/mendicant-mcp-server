...
import { feedbackLoop, ExecutionFeedback } from './feedback_loop.js';
...
export class AdaptiveExecutor {
  ...
  private adaptation_history: Adaptation[] = [];
  private recovery_patterns: Map<string, RecoveryStrategy[]> = new Map();
   ...
   */
  async processAgentResult(result: AgentResult): Promise<ExecutionState> {
    ...
    this.execution_state.pending_agents = this.execution_state.pending_agents.filter(
      a => a !== result.agent_id
    );
   ...
   */
  private async handleFailure(failed_result: AgentResult): Promise<void> {
    if (!this.execution_state) return;
   ...
   */
  private async findRecoveryStrategy(
    failed_agent: AgentId,
    error: string
    ...
    if (learned_strategies && learned_strategies.length > 0) {
      ...
      const best_strategy = learned_strategies.reduce((best, current) =>
   ...
   */
  private async findAlternativeAgents(failed_agent: AgentId): Promise<AgentId[]> {
   ...
   */
  private async applyRecoveryStrategy(strategy: RecoveryStrategy): Promise<void> {
    if (!this.execution_state) return;
   ...
   */
  private async learnRecoveryPattern(
    failed_agent: AgentId,
    error: string,
   ...
   */
  private async shouldOptimizePlan(result: AgentResult): Promise<boolean> {
    if (!this.execution_state) return false;
   ...
   */
  private async optimizePlan(): Promise<void> {
    if (!this.execution_state) return;
   ...
   */
  private removeRedundantAgents(agents: AgentId[]): AgentId[] {
    // Simple heuristic: remove duplicate agent types
    const seen = new Set<string>();
    const optimized: AgentId[] = [];
