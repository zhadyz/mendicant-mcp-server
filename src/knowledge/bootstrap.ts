...
import type { ExecutionPattern, AgentResult, ProjectContext, Conflict, Gap } from '../types.js';
import { COMMON_PATTERNS } from './patterns.js';

// Known agents with their typical token usage for bootstrap generation
const AGENT_TOKEN_USAGE: Record<string, number> = {
  'hollowed_eyes': 50000,
  'loveless': 30000,
  'the_architect': 45000,
  'the_didact': 35000,
 ...
 */
export function generateBootstrapPatterns(count: number = 100): ExecutionPattern[] {
  ...
  for (const patternKey of patternTypes) {
    ...
    for (let i = 0; i < patternsPerType; i++) {
      ...
      const syntheticPattern = generatePatternExecution(
 ...
 */
function generatePatternExecution(
  agentIds: string[],
  objective: string,
 ...
 */
function generateDiversePatterns(count: number): ExecutionPattern[] {
  const patterns: ExecutionPattern[] = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    patterns.push(generatePatternExecution(
      combo.agents,
      combo.objective,
      projectContext,
      shouldSucceed,
 ...
 */
function generateFailureReason(agentId: string, context: ProjectContext): string {
  ...
  const templates = (failureTemplates as Record<string, string[]>)[agentId] || [
 ...
 */
function randomProjectContext(): ProjectContext {
 ...
 */
function generateRandomConflicts(): Conflict[] {
  if (Math.random() < 0.7) return []; // Most executions have no conflicts
 ...
 */
function generateRandomGaps(): Gap[] {
  if (Math.random() < 0.6) return []; // Many executions have no gaps
  
  const possibleGaps: Gap[] = [
    {
 ...
 */
function extractObjectiveType(objective: string): string {
 ...
 */
function extractTags(objective: string, context: ProjectContext): string[] {
  const tags: string[] = [];
  const lower = objective.toLowerCase();
  ...
  if (lower.includes('machine learning') || lower.includes('ml')) tags.push('ml');
