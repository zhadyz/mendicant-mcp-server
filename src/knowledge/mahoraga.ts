...
export class PredictiveSelector {
  ...
  private calculatePredictiveScore(agentId: AgentId, similarPatterns: PatternMatch[]): PredictiveScore {
    ...
    const patternsWithAgent = temporalPatterns.filter(m =>
...
export class AdaptivePlanner {
  ...
  async refinePlan(
    ...
    const conflictAnalysis = await conflictDetector.analyzeConflicts(
      originalPlan.agents.map(a => a.agent_id),
      objective,
      semanticEmbedding,
      projectContext
    );
    ...
    if (alternatives.length > 1) {
      try {
        const paretoResult = await paretoOptimizer.optimize(
          ...
          semanticEmbedding,
          projectContext,
          temporalPatterns.map(m => m.pattern)
        );
    ...
    if (conflictAnalysis.recommended_reordering) {
      ...
      refinedAgents = refinedAgents
        .sort((a, b) => {
          const aIdx = reordered.indexOf(a as any);
          const bIdx = reordered.indexOf(b as any);
    ...
    if (conflictAnalysis.agents_to_remove && conflictAnalysis.agents_to_remove.length > 0) {
      refinedAgents = refinedAgents.filter(a => !conflictAnalysis.agents_to_remove!.includes(a as any));
    }

    // PHASE 5: Calculate final confidence using Bayesian engine
    const finalConfidence = bayesianEngine.calculateConfidence(
      refinedAgents as any[],
      semanticEmbedding,
      projectContext,
      temporalPatterns.map(m => m.pattern)
    );
    ...
    const refinedPlan = {
      ...
      agents: refinedAgents.map(agentId => {
        ...
        return existing || { agent_id: agentId } as any;
      }).filter(Boolean)
    };
