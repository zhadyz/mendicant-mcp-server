...
import type { AgentCapability, AgentId, ProjectContext } from '../types.js';
...
export class IntelligentSelector {
   ...
   */
  async selectAgentsForObjective(
    objective: string,
  ...
  async scoreAgent(
    ...
    semanticAnalysis: ObjectiveAnalysis
  ): Promise<AgentRecommendation> {
  ...
  private calculateSemanticScore(
    ...
    if (intentMatch && domainMatch) return 0.6;
    if (intentMatch || domainMatch) return 0.3;
   ...
   */
  private calculateHistoricalScore(perf: {
    similar_objectives: number;
    success_in_similar: number;
    avg_tokens_in_similar: number;
  }): number {
   ...
   * Extract technology keywords from objective
   */
  private extractTechKeywords(objective: string): string[] {
