...
import {
  createMnemosyneEntities,
  createMnemosyneRelations,
  semanticSearchMnemosyne
} from '../integration/mnemosyne_mcp.js';
...
export class MemoryBridge {
  ...
  private consolidationThreshold = 0.65; // Only persist memories scoring above this
  private minSuccessRate = 0.7; // Execution patterns must succeed at least 70%
  private noveltyBonus = 0.15; // Boost score for novel patterns
  ...
  constructor(
    private mnemosyneClient?: any // MCP client for Mnemosyne tools
  ) {}
   ...
   */
  async consolidateExecutionPattern(
    objective: string,
    ...
    const { entities, relations } = this.buildExecutionGraph(
      objective,
      plan,
      results,
      conflicts,
  ...
  async consolidateFailure(
    ...
    // Build failure graph
    const { entities, relations } = this.buildFailureGraph(
      objective,
      failedAgent,
      error,
      errorCategory,
   ...
   */
  async retrieveRelevantPatterns(context: RetrievalContext): Promise<ExecutionPattern[]> {
   ...
   */
  async retrieveRelevantFailures(context: RetrievalContext): Promise<FailureContext[]> {
    if (!this.mnemosyneClient) {
      return [];
    }
  ...
  private scoreExecutionMemory(
    ...
    const value = (
      success_rate * 0.35 +      // Success is critical
   ...
   */
  private isCriticalFailure(errorCategory: string): boolean {
   ...
   */
  private scoreNovelty(agents: any[]): number {
   ...
   */
  private scoreImpact(results: AgentResult[], conflicts: Conflict[], gaps: Gap[]): number {
   ...
   */
  private scoreConflictSeverity(conflicts: Conflict[]): number {
   ...
   */
  private explainScore(value: number, factors: MemoryScore['factors']): string {
   ...
   */
  private buildRetrievalQuery(context: RetrievalContext): string {
   ...
   */
  private parseExecutionPatterns(results: any[]): ExecutionPattern[] {
   ...
   */
  private parseFailureContexts(results: any[]): FailureContext[] {
   ...
   */
  adaptThresholds(feedback: { retrieval_helped: boolean, memory_reused: boolean }) {
