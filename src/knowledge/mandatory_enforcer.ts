 ...
 */
export async function checkMandatoryAgents(
  objective: string,
  currentAgents: AgentSpec[],
  projectContext?: ProjectContext
): Promise<MandatoryAgentCheck[]> {
  const checks: MandatoryAgentCheck[] = [];
 ...
 */
async function shouldIncludeOracle(objective: string, currentAgents: AgentId[]): Promise<string | null> {
  ...
  if (
    /\b(delete|remove|drop|destroy)\b.*\b(database|table|production|all|everything)\b/.test(lower)
  ) {
 ...
 */
async function shouldIncludeLibrarian(objective: string, currentAgents: AgentId[]): Promise<string | null> {
 ...
 */
async function shouldIncludeLoveless(objective: string, currentAgents: AgentId[]): Promise<string | null> {
 ...
 */
export async function addMandatoryAgents(
  agents: AgentSpec[],
  mandatoryChecks: MandatoryAgentCheck[],
  objective: string,
  ...
  for (const check of mandatoryChecks) {
    if (!check.must_include) continue;
 ...
 */
function generateMandatoryPrompt(
  agentId: AgentId,
  objective: string,
  reason: string,
  context?: ProjectContext
 ...
 */
function determineDependencies(agentId: AgentId, currentAgentIds: AgentId[]): string[] {
  if (agentId === 'the_oracle') {
