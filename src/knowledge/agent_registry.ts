...
export class AgentRegistry {
  ...
  private hardcodedAgents: Map<AgentId, AgentCapability> = new Map();

  // Tier 2: MCP-discovered agents
  private mcpDiscoveredAgents: Map<AgentId, AgentCapability> = new Map();

  // Tier 3: Mnemosyne-learned agents (via cache)
  private learnedAgentsCache: CacheLayer<LearnedAgent>;

  // Tier 4: Runtime feedback (session-only)
  private runtimeFeedback: Map<AgentId, LearnedAgent> = new Map();
  ...
  constructor() {
    ...
    this.learnedAgentsCache = new CacheLayer<LearnedAgent>('learned_agents');
    ...
    this.initialize().catch(err =>
      console.error('[AgentRegistry] Failed to initialize:', err)
    );
  }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.mcpRefreshInterval = setInterval(
        () => this.refreshMCPAgents().catch(err =>
          console.error('[AgentRegistry] MCP refresh failed:', err)
        ),
        5 * 60 * 1000
      );
  ...
  async getAgent(agentId: AgentId): Promise<AgentCapability | null> {
    ...
    if (!baseAgent && !learned) return null;

    if (!baseAgent) return this.learnedToCapability(learned!);
  ...
  async recordFeedback(feedback: AgentFeedback): Promise<void> {
    ...
    this.storeInMnemosyne(feedback).catch(err =>
      console.error('[AgentRegistry] Failed to store in Mnemosyne:', err)
    );
   ...
   */
  private async refreshMCPAgents(): Promise<void> {
    try {
   ...
   */
  private async getLearnedAgent(agentId: AgentId): Promise<LearnedAgent | undefined> {
    try {
   ...
   */
  private async getAllLearnedAgentIds(): Promise<AgentId[]> {
