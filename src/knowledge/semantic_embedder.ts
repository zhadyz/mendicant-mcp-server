...
export interface SemanticEmbedding {
  objective: string;
  // Placeholder for actual embedding vector (1536 dimensions for Claude)
  // In production, use @anthropic/sdk or similar to get real embeddings
  vector_placeholder: string;
  
  // Multi-label intent scores (objectives can have multiple intents)
  intent_scores: Map<Intent, number>;
  
  // Multi-label domain scores (objectives can span multiple domains)
...
export class SemanticEmbedder {
  ...
  private feedback_history: FeedbackRecord[] = [];
  
  // Intent/Domain classification weights (learned from feedback)
  private intent_weights: Map<Intent, Map<string, number>> = new Map();
  private domain_weights: Map<Domain, Map<string, number>> = new Map();
   ...
   */
  async analyzeObjective(objective: string): Promise<SemanticEmbedding> {
   ...
   */
  private classifyIntents(objective: string): Map<Intent, number> {
  ...
  private classifyTaskTypes(
    ...
    // COMMUNICATIVE
    scores.set('communicative', Math.max(
      domain_scores.get('documentation')!,
      intent_scores.get('document')!
    ));
    
    // ANALYTICAL
    scores.set('analytical', Math.max(
      intent_scores.get('investigate')!,
      domain_scores.get('research')!
    ));
    
    // OPERATIONAL
    scores.set('operational', Math.max(
      domain_scores.get('infrastructure')!,
      intent_scores.get('deploy')!
    ));
    
    // TECHNICAL
    scores.set('technical', Math.max(
      domain_scores.get('code')!,
      domain_scores.get('testing')!,
      domain_scores.get('data')!,
      intent_scores.get('create_new')! * 0.8,
      intent_scores.get('modify_existing')! * 0.8
    ));
   ...
   * Estimate complexity
   */
  private estimateComplexity(objective: string): number {
    const words = objective.split(/\s+/).length;
    let score = 0.0;
    
    // Length-based
    if (words <= 5) score += 0.2;
    else if (words <= 10) score += 0.4;
    else if (words <= 20) score += 0.6;
    else score += 0.8;
