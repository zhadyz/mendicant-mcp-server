...
export interface BayesianConfidence {
  ...
  // Calibration quality
  calibration_score: number; // How well-calibrated are our predictions
...
export interface EvidenceSource {
  source: string;
  likelihood: number;   // P(evidence | success)
  prior: number;        // P(success)
  posterior: number;    // P(success | evidence)
...
export class BayesianConfidenceEngine {
  ...
  private agent_priors: Map<AgentId, number> = new Map();
  
  // Calibration history
  private calibration_history: CalibrationPoint[] = [];
  
  // Calibration curve (isotonic regression coefficients)
  private calibration_curve: Map<number, number> = new Map();
  ...
  calculateConfidence(
    ...
    const evidence_breakdown: EvidenceSource[] = [];
    ...
    const objective_evidence = this.computeObjectiveEvidence(
      objective_embedding,
      execution_history
    );
  ...
  private computeAgentEvidence(
    ...
    const relevant_patterns = history.filter(p =>
      agents.every(agent => p.agents_used.includes(agent))
    );
  ...
  private computeObjectiveEvidence(
    ...
    const similar_patterns = history.filter(p => {
      ...
      return (
        top_intents.some(intent => obj_type.includes(intent) || tags.includes(intent)) ||
  ...
  private computeContextEvidence(
    ...
    const similar_patterns = history.filter(p => {
      if (!p.project_context) return false;
   ...
   */
  private computeHistoricalVariance(history: ExecutionPattern[]): { mean: number; variance: number } {
    if (history.length === 0) {
   ...
   */
  private applyCalibration(raw_confidence: number): number {
   ...
   */
  private rebuildCalibrationCurve(): void {
   ...
   */
  private assessCalibrationQuality(): number {
    if (this.calibration_history.length < 10) {
