 ...
 */
export function detectVagueRequest(objective: string): VagueAnalysis {
  const lower = objective.toLowerCase().trim();
  const words = lower.split(/\s+/);

  const vagueness_indicators: string[] = [];
  ...
  if (vagueness_indicators.some(i => i.startsWith('ultra_vague_phrase'))) {
    ...
    if (words.length < 3) vagueScore += 0.3;
    else if (words.length < 5) vagueScore += 0.15;
 ...
 */
export function shouldInvokeLibrarian(vagueAnalysis: VagueAnalysis): boolean {
