...
export interface Message {
  ...
  parent_message_id?: string; // For request/response chains
...
export type MessageType =
  ...
  | 'coordination'   // Coordination message (work distribution)
...
export class AgentCommunicationBus {
  private message_queues: Map<AgentId, MessageQueue> = new Map();
  private message_history: Message[] = [];
  private pending_responses: Map<string, (response: Message) => void> = new Map();
  ...
  async sendRequest(
    ...
    options?: { priority?: Priority; metadata?: Record<string, any> }
  ): Promise<Message> {
   ...
   */
  async receiveMessage(agent_id: AgentId): Promise<Message | null> {
  ...
  getMessageHistory(options?: {
    ...
    if (options?.agent_id) {
      history = history.filter(
        m => m.from === options.agent_id || m.to === options.agent_id
      );
    }
   ...
   */
  getQueueStatus(): Array<{ agent_id: AgentId; message_count: number; processing: boolean }> {
   ...
   */
  private sortQueueByPriority(queue: MessageQueue): void {
