...
export class EmbeddingService {
     ...
     */
    constructor(provider, fallbackEnabled = true) {
    ...
    async initialize() {
        ...
        if (this.fallbackEnabled) {
            console.warn('[EmbeddingService] No embedding provider available (Mnemosyne or OpenAI). ' +
                'Using keyword-based fallback only. ' +
                'For better results: configure Mnemosyne MCP or set OPENAI_API_KEY.');
        }
        else {
            console.error('[EmbeddingService] No embedding provider available and fallback disabled. ' +
     ...
     */
    async getOrGenerateEmbedding(text) {
    ...
    cosineSimilarity(a, b) {
        if (a.length !== b.length) {
            console.warn(`[EmbeddingService] Dimension mismatch: ${a.length} vs ${b.length}. ` +
