 ...
 */
export class MnemosyneEmbeddingProvider {
    dimensions = 1024; // BGE-large embedding size
    available = false;
    tempEntityPrefix = 'temp_embedding_';
    /**
    ...
    async generateEmbedding(text) {
        if (!this.available) {
            throw new Error('Mnemosyne not available for embeddings. ' +
        ...
        try {
            ...
            if (embedding.length !== this.dimensions) {
                console.warn(`[MnemosyneEmbeddingProvider] Expected ${this.dimensions} dimensions, got ${embedding.length}`);
            }
            return embedding;
        }
        catch (error) {
     ...
     * @private
     */
    async checkMnemosyneAvailability() {
        try {
     ...
     * @private
     */
    async createEntity(entityName, text) {
        throw new Error('createEntity must be executed by Claude Code with Mnemosyne MCP tools. ' +
     ...
     * @private
     */
    async getEmbedding(entityName) {
        throw new Error('getEmbedding must be executed by Claude Code with Mnemosyne MCP tools. ' +
            'This TypeScript code documents the interface but cannot directly call MCP tools. ' +
    ...
    async deleteEntity(entityName) {
        ...
        console.log(`[MnemosyneEmbeddingProvider] Would delete temp entity: ${entityName}. ` +
            `Expected call: mcp__mnemosyne__delete_entities({ entityNames: ["${entityName}"] })`);
    }
}
//# sourceMappingURL=mnemosyne_provider.js.map