 ...
 */
function detectSubType(category, errorMessage) {
    const lower = errorMessage.toLowerCase();
    switch (category) {
        case 'version_mismatch':
            if (/using.*v\d+.*expects.*v\d+/i.test(lower) || /version.*conflict/i.test(lower))
                return 'version_conflict';
            if (/incompatible.*version/i.test(lower))
                return 'version_conflict';
            if (/deprecated/i.test(lower))
                return 'version_conflict';
            return 'version_conflict';
        case 'missing_dependency':
            if (/peer.*dependency/i.test(lower))
                return 'peer_dependency_missing';
            if (/version.*conflict/i.test(lower) || /incompatible/i.test(lower))
                return 'version_conflict';
            if (/transitive/i.test(lower) || /indirect/i.test(lower))
                return 'transitive_dependency_missing';
            return 'direct_dependency_missing';
        case 'type_error':
            if (/null/i.test(lower))
                return 'null_reference';
            if (/undefined.*property/i.test(lower))
                return 'undefined_property';
            if (/type.*guard/i.test(lower))
                return 'type_guard_failure';
            return 'type_mismatch';
        case 'network_error':
            if (/econnrefused/i.test(lower) || /connection.*refused/i.test(lower))
                return 'connection_refused';
            if (/timeout/i.test(lower) || /etimeout/i.test(lower))
                return 'connection_timeout';
            if (/dns/i.test(lower) || /getaddrinfo/i.test(lower))
                return 'dns_resolution_failed';
            if (/ssl/i.test(lower) || /certificate/i.test(lower))
                return 'ssl_certificate_error';
            return 'connection_refused';
        case 'configuration_error':
            if (/env.*not.*set/i.test(lower) || /missing.*env/i.test(lower))
                return 'missing_env_var';
            if (/invalid.*value/i.test(lower) || /wrong.*type/i.test(lower))
                return 'invalid_config_value';
            if (/not.*found/i.test(lower) || /enoent/i.test(lower))
                return 'config_file_not_found';
            if (/parse/i.test(lower) || /invalid.*json/i.test(lower))
 ...
 */
function detectDomain(category, errorMessage) {
    const lower = errorMessage.toLowerCase();
 ...
 */
function assessSeverity(category, errorMessage) {
    const lower = errorMessage.toLowerCase();
 ...
 */
function determineRecoveryStrategy(category, severity, errorMessage) {
    const lower = errorMessage.toLowerCase();
 ...
 */
function estimateRecoveryTime(category, strategy) {
    if (strategy === 'retry')
        return 1;
...
export function classifyError(errorMessage) {
    ...
    else if (/auth/i.test(lower) && /fail/i.test(lower) ||
    ...
    else if (/cannot find module/i.test(lower) ||
    ...
    else if (/type.*error/i.test(lower) ||
    ...
    else if (/syntax.*error/i.test(lower) ||
    ...
    else if (/compil.*error/i.test(lower) ||
    ...
    else if (/test.*fail/i.test(lower) ||
    ...
    else if (/network.*error/i.test(lower) ||
    ...
    else if (/database.*error/i.test(lower) ||
    ...
    else if (/config.*error/i.test(lower) ||
    ...
    else if (/out of memory/i.test(lower) ||
    ...
    else if (/not found/i.test(lower) ||
