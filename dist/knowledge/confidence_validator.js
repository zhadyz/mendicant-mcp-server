 ...
 */
export function validateConfidence(plan, semanticConfidence, mahoragaConfidence, patternConfidence, thresholds = DEFAULT_THRESHOLDS) {
    ...
    const hasUnknownAgents = plan.agents.some(spec => typeof spec === 'object' && spec.agent_id.startsWith('unknown_'));
    if (!hasUnknownAgents) {
        confidence_sources.push({
    ...
    const recommendations = generateConfidenceRecommendations(overall_confidence, confidence_sources, mergedThresholds, plan);
    return {
        should_execute,
        overall_confidence,
        confidence_level,
 ...
 */
function generateConfidenceRecommendations(confidence, sources, thresholds, plan) {
 ...
 */
export function shouldAddFallbackAgents(confidence, thresholds = DEFAULT_THRESHOLDS) {
    const mergedThresholds = {
        minimum_overall: thresholds?.minimum_overall ?? DEFAULT_THRESHOLDS.minimum_overall,
        warning_threshold: thresholds?.warning_threshold ?? DEFAULT_THRESHOLDS.warning_threshold,
        high_confidence: thresholds?.high_confidence ?? DEFAULT_THRESHOLDS.high_confidence,
 ...
 */
export function suggestFallbackAgents(confidence, plan) {
 ...
 */
export function getConfidenceMessage(analysis) {
...
//# sourceMappingURL=confidence_validator.js.map