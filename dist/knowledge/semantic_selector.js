 ...
 */
export function analyzeObjectiveSemantic(objective) {
...
function detectIntent(objective) {
    ...
    if (/\b(research|investigate|explore|understand|learn|find out|discover|analyze)\b/.test(objective) ||
        /^(what|how|why|where|when)\b/.test(objective)) {
        return 'investigate';
    }
    // VALIDATE - Testing and verification
    if (/\b(test|verify|validate|check|ensure|confirm|audit)\b/.test(objective) &&
        !/\b(create|build|implement)\b/.test(objective)) {
        return 'validate';
    }
    // FIX_ISSUE - Bug fixing and problem solving
    if (/\b(fix|repair|resolve|solve|debug|patch|correct)\b/.test(objective) ||
    ...
    if (/\b(design|architect|plan|blueprint|structure)\b/.test(objective) &&
 ...
 */
function detectDomain(objective, intent) {
    ...
    if (/\b(security|vulnerability|vulnerabilities|exploit|cve|penetration|audit)\b/.test(objective) &&
    ...
    if (/\b(test|testing|qa|quality assurance|e2e|integration|unit)\b/.test(objective) &&
        intent === 'validate') {
        return 'testing';
    }
    ...
    if (/\b(database|sql|nosql|postgres|mongo|data|dataset|query|schema)\b/.test(objective)) {
        return 'data';
    }
    // DOCUMENTATION - Technical writing
    if (/\b(docs|documentation|readme|guide|tutorial|manual|wiki)\b/.test(objective) ||
        intent === 'document') {
        return 'documentation';
    }
    // ARCHITECTURE - System design
    if (/\b(architecture|design|pattern|structure|blueprint|system design)\b/.test(objective) &&
        intent === 'design') {
        return 'architecture';
    }
    ...
    if (/\b(code|implement|develop|build|function|class|api|endpoint|feature)\b/.test(objective) ||
        intent === 'create_new' ||
 ...
 */
function detectTaskType(objective, intent, domain) {
 ...
 */
function detectComplexity(objective) {
 ...
 */
function recommendAgents(intent, domain, task_type, complexity, objective) {
    const agents = [];
    let reasoning = '';
 ...
 */
export function getCapabilitiesFromAnalysis(analysis) {
    const capabilities = [];
...
//# sourceMappingURL=semantic_selector.js.map