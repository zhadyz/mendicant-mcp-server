...
export class ParetoOptimizer {
     ...
     */
    async optimize(objective, candidate_plans, objective_embedding, context, execution_history) {
        console.log(`[ParetoOptimizer] Optimizing ${candidate_plans.length} candidate plans`);
        // Evaluate all candidates
        const candidates = await this.evaluateCandidates(candidate_plans, objective_embedding, context, execution_history);
     ...
     */
    async evaluateCandidates(plans, objective_embedding, context, execution_history) {
        const candidates = [];
        for (let i = 0; i < plans.length; i++) {
            const agents = plans[i];
            const plan_id = `plan_${i}`;
            // Estimate accuracy (from Bayesian confidence)
            const confidence_result = bayesianEngine.calculateConfidence(agents, objective_embedding, context, execution_history || []);
    ...
    findParetoFrontier(candidates) {
        ...
        for (const candidate of candidates) {
            ...
            if (!is_dominated) {
                ...
                candidate.dominance_count = candidates.filter(c => c.plan_id !== candidate.plan_id && this.dominates(candidate, c)).length;
     ...
     */
    selectBestPlan(frontier, preferences) {
     ...
     */
    calculateUtility(plan, preferences) {
        ...
        const utility = accuracy * norm_accuracy_w +
            (1 - cost) * norm_cost_w +
     ...
     */
    explainChoice(chosen, frontier, preferences) {
     ...
     */
    async learnPreferences(chosen_plan, rejected_plans) {
        ...
        this.user_preferences.preference_strength = Math.min(1.0, this.user_preferences.preference_strength + 0.05);
...
//# sourceMappingURL=pareto_optimizer.js.map