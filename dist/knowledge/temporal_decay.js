...
export class TemporalDecayEngine {
     ...
     */
    enrichPattern(pattern, current_time = Date.now()) {
     ...
     */
    calculateDecayedScore(pattern, current_time, half_life_days) {
     ...
     */
    estimateContextStability(pattern) {
     ...
     */
    filterByRelevance(patterns, threshold = 0.3, current_time = Date.now()) {
     ...
     */
    sortByRelevance(patterns, current_time = Date.now()) {
        const enriched = patterns.map(p => this.enrichPattern(p, current_time));
     ...
     */
    computeWeightedRelevance(pattern, current_time = Date.now()) {
     ...
     */
    getRelevantPatternsInWindow(patterns, window_days, current_time = Date.now()) {
     ...
     */
    predictIrrelevanceDate(pattern, relevance_threshold = 0.1) {
     ...
     */
    batchEnrich(patterns, current_time = Date.now()) {
     ...
     */
    calculateTemporalHealth(patterns, current_time = Date.now()) {
        ...
        const health_score = Math.max(0, Math.min(1.0, (age_score * 0.3 + relevance_score * 0.4 + freshness_ratio * 0.3) - staleness_penalty));
        return {
            average_age_days,
            average_relevance,
            fresh_patterns,
...
//# sourceMappingURL=temporal_decay.js.map